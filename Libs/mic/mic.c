...
void start_recording() {
  if (f_open(&audioFile, latest_audio_filename, FA_READ | FA_CREATE_ALWAYS) == FR_OK) {
      my_printf("start_recording\r\n");
      write_wav_header(&audioFile);
      recording = 1;
      total_samples = 0;

      HAL_I2S_Receive_DMA(&hi2s1, (uint16_t *)i2s_rx_buffer, I2S_BUFFER_SIZE * 2);
  }
}

void stop_recording() {
    my_printf("stop_recording\r\n");
    recording = 0;
...
void write_wav_header(FIL *file) {
    ...
    my_printf("write_wav_header done\r\n");
}

void append_pcm_data(FIL *file, int32_t *data, uint32_t size) {
  UINT bytesWritten;
    *data = (*data >> 8);
